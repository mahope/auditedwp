...
- Næste opgave: **10 — Forbedr konvertering efter ærlig baseline**. Opgave 5 (kunderapporter) kræver svar på spørgsmål 1 og må derfor ikke begyndes.
...
- `VERIFICÉR DEPLOY: kvalitetsgate + UA-fix i scanneren <sha> 2026-09-25 19:45 UTC` — workflowen deployer `site-dist/` uændret af denne diff (ingen `site/**`-fil rørtes), men **de to nye scripts og den nye `verify.yml` skal køre grønt i CI, før næste deploy**. Verificér på indhold: (1) den nye produktionskontrol skal have kørt grønt og rapporteret de 12 interne stier som 404 plus de 13 nøglesider som 200 — hvis den fejler, er det fordi en ny fil er sluppet ind i det publicerede træ; (2) `https://eucomplypro.com/assets/eucomply-1.3.2.zip` skal stadig være 200 og en zip; (3) intet site-indhold ændrer sig ved denne diff, så 322 filer er uændrede kopier af det der lå i `site/` før. Den publicerede npm-CLI kan **ikke** rettes uden en ny `npm publish`, som agenten ikke må lave — den kræver Mads (spørgsmål 13).
